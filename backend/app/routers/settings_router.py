...
async def _get_settings_doc() -> dict:
    ...
    if not doc:
        doc = {
            ...
            "email_from_address": "", "language": "zh-CN", "theme": "light",
...
async def get_settings():
    ...
    return SettingsResponse(
        ...
        email=EmailSettings(
            ...
            smtp_password="", from_address=doc.get("email_from_address", ""),
        ),
...
async def update_settings(update: SettingsUpdate):
    ...
    await collection.update_one(
        {"key": SETTINGS_KEY}, {"$set": update_data}, upsert=True,
    )
...
async def test_api_connection(req: APITestRequest):
    ...
    try:
        ...
        return APITestResponse(success=True, message=f"连接成功！模型回复: {reply}", model=model)
    except Exception as e:
        return APITestResponse(success=False, message=f"连接失败: {str(e)}")
